...
var fatigue = 0;
var massFatigue = 0;
var fatigueDeath = false;
var fatigueColor = false;
var tired = false;
var playerDamage = 0;
var anticipationDMG = 0;
var foresight = false;
  ...
  if (!gameStart) {
    ...
    for (var i = 0, length = radios.length; i < length; i++) {
     if (radios[i].checked) {
        ...
        var initMsg = "You enter the dungeon with your "+sideNum+"-sided die!<br /><br />";
...
function rollDie() {
  ...
  if (sideNum == 4) {
    ...
    playerDamage = Math.floor(((Math.random() * 8) + 1) * d8totalstocks);
    d8totalstocks = 0;
    d8limit = 2;
  } else if (d12recoil) {
  ...
  //D8 effect
    ...
    if (defense) {
      ...
      if (d8stockpile >= d8limit) {
        ...
        d8limit += 0.5;
        d8trigger = true;
        effectMsg = "D-8 stockpiled energy for a total of "+d8totalstocks+"! <br>Its next roll will expend energy for "+d8totalstocks+"x damage!";
      }
    }
  //D12 effect
...
function defendRecolor() {
  if (fatigueColor) {
    document.getElementById("defendButton").style.color = "black";
    document.getElementById("defendButton").style.borderColor = "black";
    document.getElementById("defendButton").style.backgroundColor = "white";
  } else if (fatigue <= 0 ) {
    document.getElementById("defendButton").style.color = "#00ffff";
    document.getElementById("defendButton").style.borderColor = "#00ffff";
    document.getElementById("defendButton").style.backgroundColor = "#000099";
  } else if (fatigue > 0 && fatigue <= 1) {
    document.getElementById("defendButton").style.color = "yellow";
    document.getElementById("defendButton").style.borderColor = "yellow";
    document.getElementById("defendButton").style.backgroundColor = "#ff6600";
  } else if (fatigue > 1 && fatigue <= 2) {
    document.getElementById("defendButton").style.color = "red";
    document.getElementById("defendButton").style.borderColor = "red";
    document.getElementById("defendButton").style.backgroundColor = "#800000";
  }
}
...
function damageCalc() {
  ...
  enemyAttack();
  if (recoilCalc) {
    effectMsg = "D-12 damaged itself! It took an additional "+playerDamage+" damage!<br>";
    anticipationDMG+=playerDamage;
    playerDamage = 0;
  }
  if (defense && (!tired)) {
    combatMsg = "You defend against the "+enemyData[0]+"'s attack of "+anticipationDMG+" damage!<br /><br />";
    if (d8trigger) {
      d8trigger = false;
    ...
    restoreHP();
    if (!tired && sideNum !== 20) {
      combatMsg = "You attacked the "+enemyData[0]+" for "+playerDamage+" damage and restored "+playerRestore+" HP!<br> It fights back with "+anticipationDMG+" damage!";
      if (d20attempt) {
        combatMsg = "D-20 took "+anticipationDMG+" damage from the "+enemyData[0]+"<br /><br />";
        playerDamage = 0;
        d20attempt = false;
        tired = false;
        fatigue = 0;
      ...
      combatMsg = "D-"+sideNum+" got tired of defending and took "+anticipationDMG+" damage!<br /><br />";
      tired = false;
      massFatigue+=1;
...
function endOfTurn() {
  ...
  if (isGameOver) {
    ...
    if (fatigueDeath) {
      fatigueColor = true;
      defendRecolor();
      combatMsg = "You have perished due to fatigue! Press restart to try again!<br /><br />";
      fatigue = 0;
    } else {
      combatMsg = "The "+enemyData[0]+" slayed you! Press restart to try again!<br /><br />";
    }
    effectMsg = "<span style='color:yellow'>Floor: </span> "+enemyTypeFloor+" <span style='color:red'>HP: </span> "+playerHp;
  } else if (!enemyAppeared) {
    summon();
    effectMsg = "Next Enemy: "+enemyData[0]+" <span style='color:yellow'>Floor: </span> "+enemyTypeFloor+" <span style='color:red'>HP: </span> "+playerHp+"<br> <span style='color:fuchsia'>Anticipated enemy damage: </span> [??]";
    anticipate();
    mystery();
  } else {
    anticipate();
